...
### MVP
- [ ] `devstories.quickCapture` command registered
- [ ] Keybinding: `Cmd+Shift+S` (configurable)
- [ ] Shows single input box for title
- [ ] Creates story with defaults: type=task, size=M, current sprint
- [ ] Does NOT open file (stays in current context)
- [ ] Shows brief notification: "Created DS-XXX: {title}"
- [ ] Tree view updates immediately

### Enhanced
- [ ] **Inbox epic**: Auto-create `EPIC-INBOX` if doesn't exist, route quick captures there
- [ ] **Pipe syntax**: Support `title | notes` format for quick context
  - "Fix login | users report 500 on submit" → title + pre-filled description
- [ ] **From selection**: If text selected in editor, use as title (truncate if >100 chars)
- [ ] **Type shorthand**: Prefix parsing for quick type assignment:
  - `bug: Login fails` → type=bug
  - `feat: Dark mode` → type=feature
  - `chore: Update deps` → type=chore
- [ ] **Repeat capture**: After creation, option to "Capture another?" (quick succession mode)
