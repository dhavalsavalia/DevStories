...
### MVP
- [x] Reads `templates` section from `config.yaml`
- [x] Applies template based on story type (feature/bug/task/chore)
- [x] Substitutes variables: `{{DATE}}`, `{{TITLE}}`, `{{ID}}`
- [x] Falls back to built-in default if config template missing
- [x] Supports multi-line YAML templates

### Enhanced
- [x] **Bundled template library**: Pre-built templates for common patterns:
...
- [x] **Template picker**: When creating story, option to "Use template..." shows library
- [x] **Save as template**: Right-click story in tree → "Save as Template"
  - Saves to `.devstories/templates/` as reusable `.md` file
- [x] **Template variables**: Extended variable support:
  - `{{DATE}}` - Today's date
  - `{{PROJECT}}` - From config.yaml
  - `{{AUTHOR}}` - From git config or VS Code setting
  - `{{CURSOR}}` - Where cursor lands after creation (not implemented - low priority)
