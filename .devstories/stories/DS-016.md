...
### MVP
- [ ] Regex correctly identifies `[[ID]]` pattern in document text
- [ ] `LinkResolver.resolve(id)` returns absolute file path
- [ ] Handles case where ID does not exist (returns null)
- [ ] Supports both Story and Epic IDs
- [ ] Works with custom ID prefixes from config

### Enhanced
- [ ] **Broken link detection**: Identify `[[INVALID-ID]]` that don't resolve
