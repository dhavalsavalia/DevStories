...
- Auto-refresh dependent views (status bar, board, tree labels) when config changes—no manual reload.

## Acceptance Criteria
- [x] Editing `config.yaml` (statuses, sprints, sizes, templates) updates commands and UI within ~250ms.
- [x] Adding/editing template files under `.devstories/templates/` updates the create-story template list automatically.
- [x] Board columns re-render when statuses change; tree view and pickers rebuild options.
- [x] Config parsing failures surface a VS Code warning but fall back to last-known-good values.
- [x] Unit tests cover watcher debounce, parsing, and consumer update hooks.
