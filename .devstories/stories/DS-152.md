...
- [ ] Create `schemas/devstories.schema.json`
- [ ] Add `contributes.jsonValidation` to `package.json`
- [ ] Verify: autocomplete works (Ctrl+Space in config.json)
- [ ] Verify: squiggly lines appear for invalid values
- [ ] Verify: hover shows field descriptions

### Config Parsing Changes
- [ ] `configServiceUtils.ts`: Replace `parseConfigYamlContent()` → `parseConfigJsonContent()`
- [ ] `configServiceUtils.ts`: Add `validateSprintConfig()` (currentSprint must exist in sequence)
- [ ] `configServiceUtils.ts`: Remove `inlineTemplates` from `ConfigData` interface
- [ ] `configService.ts`: Change watcher pattern to `**/.devstories/config.json`
- [ ] `configService.ts`: Change load path to `config.json`
- [ ] `configService.ts`: Call `validateSprintConfig()` after merge, show error if invalid
- [ ] `configServiceNotifications.ts`: Update path reference

### Init Command
- [ ] `initUtils.ts`: Replace `generateConfigYaml()` → `generateConfigJson()`
- [ ] `init.ts`: Write `config.json` instead of `config.yaml`

### Other Commands
- [ ] `createStory.ts`: Read `config.json`
- [ ] `createStoryUtils.ts`: Replace `parseConfigYaml()` → `parseConfigJson()`
- [ ] `quickCapture.ts`: Read `config.json`
- [ ] `changeStatus.ts`: Read `config.json`

### Tests
- [ ] `configService.test.ts`: JSON parsing tests, sprint validation tests
- [ ] `configService.test.ts` (integration): JSON strings
- [ ] `init.test.ts`: Test `generateConfigJson()`

### Migration
- [ ] Convert `.devstories/config.yaml` → `.devstories/config.json`
- [ ] Convert `devstories_test/.devstories/config.yaml` → `config.json`
