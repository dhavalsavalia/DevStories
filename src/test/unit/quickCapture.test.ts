...
describe('quickCaptureUtils', () => {
  describe('parseQuickInput', () => {
    it('should parse plain title', () => {
      const result = parseQuickInput('Add dark mode toggle');
    ...
    it('should parse type prefix with pipe syntax', () => {
      const result = parseQuickInput('bug: Login 500 error | Users on Safari report 500 when submitting');
    ...
    it('should handle multiple pipes (first one splits)', () => {
      const result = parseQuickInput('Task | note 1 | note 2');
      expect(result.title).toBe('Task');
      expect(result.notes).toBe('note 1 | note 2');
    });

    it('should trim whitespace', () => {
      const result = parseQuickInput('  bug:   Fix issue   |   some notes  ');
