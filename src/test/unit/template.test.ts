...
describe('Template Utils', () => {
  ...
  describe('BUNDLED_TEMPLATES', () => {
    it('should have api-endpoint template', () => {
      expect(BUNDLED_TEMPLATES['api-endpoint']).toBeDefined();
      expect(BUNDLED_TEMPLATES['api-endpoint']).toContain('Endpoint');
    });

    it('should have react-component template', () => {
      expect(BUNDLED_TEMPLATES['react-component']).toBeDefined();
      expect(BUNDLED_TEMPLATES['react-component']).toContain('Component');
    });

    it('should have db-migration template', () => {
      expect(BUNDLED_TEMPLATES['db-migration']).toBeDefined();
      expect(BUNDLED_TEMPLATES['db-migration']).toContain('Migration');
    });

    it('should have bug-investigation template', () => {
      expect(BUNDLED_TEMPLATES['bug-investigation']).toBeDefined();
      expect(BUNDLED_TEMPLATES['bug-investigation']).toContain('Root Cause');
    });
  });

  describe('resolveTemplateReference', () => {
    it('should resolve @library reference to bundled template', () => {
      const result = resolveTemplateReference('@library/api-endpoint');
      expect(result).toBe(BUNDLED_TEMPLATES['api-endpoint']);
    });

    it('should return null for unknown library template', () => {
      const result = resolveTemplateReference('@library/nonexistent');
      expect(result).toBeNull();
    });

    it('should return null for non-library strings', () => {
      const result = resolveTemplateReference('plain template content');
      expect(result).toBeNull();
    });

    it('should return null for malformed library reference', () => {
      const result = resolveTemplateReference('@library/');
      expect(result).toBeNull();
    });
  });
});
